...
//=NO CHANGE AREA ENDS=========================================================   
    ...
    // Lock shared across all Barman instances to prevent concurrent file writes
    ...
    private void recordCompletedOrder(DrinkOrder order) throws IOException {
        ...
        // Each scheduler writes to its own CSV file
        String filename = "results/" + schedulerName.trim() + ".csv";
        File file = new File(filename);
        synchronized (FILE_LOCK) {
            boolean writeHeader = !file.exists() || file.length() == 0;
            try (FileWriter fw = new FileWriter(file, true)) {
                if (writeHeader) {
                    fw.write("patron,drink,executionTime,queueLevel," +
